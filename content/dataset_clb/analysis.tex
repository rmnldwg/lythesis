...
Therefore, when we use this dataset (also in conjunction with the \gls{usz} data from \cref{chap:dataset_usz}), instead of probabilistically modelling sensitivity and specificity, we will resort to a slightly more naive approach: Using the observations for the \glspl{lnl} of a given patient along with literature values from \citeauthorandlink{de_bondt_detection_2007}, we compute the likelihoods for the possible true involvements and create a maximum likelhood consensus ``diagnosis'', which we will then use for inference with fixed values for sensitivity and specificity of 1.
