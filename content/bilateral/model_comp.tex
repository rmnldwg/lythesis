...
\begin{figure}
    ...
    \caption[
        ...
        Predicted prevalences (shaded histograms) and posterior beta distributions of observed prevalences (solid lines) for the contralateral overall involvement (anything \emph{not} N0, on that side of the neck). Predicted and observed prevalences are stratified by T-stage and mid-plane extension: Early T-stage prevalences are shown in blue and orange (and labelled \texttt{early}), late T-stage in green and red (labelled \texttt{late}). Without mid-sagittal extension (labelled \texttt{noext}) the plots are colored blue and green, whereas with the extension (label \texttt{ext}), they are colored orange and red. The three models $\mathcal{M}_\text{ag}$, $\mathcal{M}_\alpha$ and $\mathcal{M}_\text{full}$ are depicted in the left, middle and right panel respectively.
    }
...
\begin{enumerate*}[label={(\arabic*)}]
    ...
    \item late T-stage with, a lateralized tumor (green) and finally
...
\begin{figure}
    ...
    \caption[
        ...
        \begin{enumerate*}[label={(\arabic*)}]
            \item an unsuspicious \gls{ct} scan (clinically $N_0$) of all levels in both sides of the neck for an early T-stage patient whose tumor is clearly lateralized (green shaded),
            \item the same as before, but for a late T-stage patients (blue hatched),
            \item clinically $N_0$, late T-stage but now with a tumor extending over the mid-sagittal plane (blue shaded),
            \item all \glspl{lnl} appear negative on \gls{ct}, except for a positive finding in the ipsilateral \gls{lnl} II, with a late T-stage patient having a mid-sagittal extension (orange shaded), and lastly
