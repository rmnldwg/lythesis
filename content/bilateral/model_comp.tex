...
\end{equation}
%
where -- again -- $k$ is the number of parameters $\theta$, while $N$ is the number of patients in the dataset $\boldsymbol{\mathcal{D}}$. How reliable the \gls{bic} is for a given model, depends on whether its core assumptions hold:
\begin{enumerate*}[label={(\arabic*)}]
    \item the posterior must be unimodal and decay rapidly outside its maximum, while
    \item $N$ must be much larger than $k$
\end{enumerate*}
\cite{bhat_derivation_2010}. The second assumption is not quite fulfilled, but we will see shortly that the \gls{bic} seems to be a quite good measure for model comparison in our case.
