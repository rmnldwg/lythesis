...
To apply the briefly outlined concepts of this section to our model, some questions need to be addressed first. For example, exactly which iterative algorithm to implement, as many ``\gls{em}-like'' approaches have been developed, such as \emph{deterministic annealing} by \citeauthorandlink{rose_deterministic_1998}. This choice also highly depends on the performance of each implementation. It may, for instance, be computationally too expensive to sample form a distribution until convergence during each of the alternating steps that are themselves repeated until a stable solution is found.
