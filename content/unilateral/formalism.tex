...
We describe the true, but unobservable and hence \emph{hidden}, state of a \gls{lnl} using a binary \acrlong{rv} $X_v$, where $v \in \{ 1,2, \ldots, V \}$ indexes the $V$ nodal levels. If it is in state $X=0$ (\texttt{FALSE}), the corresponding \gls{lnl} is healthy. State $X=1$ (\texttt{TRUE}) means the respective \gls{lnl} contains at least one lymph node that harbors disease.
