...
\end{equation}
%
At each point of this \gls{ti} schedule, we ran the sampling for 1000 steps, simultaneously drawing $20 \cdot k$ burn-in samples, where $k$ is the number of dimensions of the model. This dimensionality $k = 3$ for the model without any connections between \gls{lnl} II and III (graph no. 1 in \cref{fig:graph:simple}), $k = 4$ for the models with one connection between these \glspl{lnl} (graphs 2 and 3 in \cref{fig:graph:simple}) and $k = 5$ for the model connecting the two levels in both directions.
...
\begin{figure}
    ...
    \caption[
        ...
        Corner plot of the distribution of the sampled parameters for the model based on graph no. 4 (blue) and for the model based on graph no. 1 (green). A corner plot represents shows all 1D and all 2D marginals of any $n$-dimensional distribution aligned.
    }
