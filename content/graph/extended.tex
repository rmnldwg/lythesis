...
\begin{tcolorbox}[title=\faIcon{recycle} Reproducibility, parbox=false]
    The base graph that will be used as a baseline for additional connections can be reproduced from the \repolink{lynference} repository, using the tagged release \href{https://github.com/rmnldwg/lynference/releases/tag/extended-base-v1}{\faIcon{tags} \texttt{extended-base-v1}}.
\end{tcolorbox}
...
\begin{tcolorbox}[title=\faIcon{recycle} Reproducibility, parbox=false]
    ...
    \begin{itemize}
        \item \textbf{III \faIcon{long-arrow-alt-right} V}: \href{https://github.com/rmnldwg/lynference/releases/tag/extended-add35-v1}{\faIcon{tags} \texttt{extended-add35-v1}}
        \item \textbf{IV \faIcon{long-arrow-alt-right} V}: \href{https://github.com/rmnldwg/lynference/releases/tag/extended-add45-v1}{\faIcon{tags} \texttt{extended-add45-v1}}
    \end{itemize}
\end{tcolorbox}
...
\begin{tcolorbox}[title=\faIcon{recycle} Reproducibility, parbox=false]
    ...
    \begin{itemize}
        \item \textbf{II \faIcon{long-arrow-alt-right} I} and \textbf{III \faIcon{long-arrow-alt-right} V}: \href{https://github.com/rmnldwg/lynference/releases/tag/extended-add21-add35-v1}{\faIcon{tags} \texttt{extended-add21-add35-v1}}
        \item \textbf{II \faIcon{long-arrow-alt-right} I} and \textbf{IV \faIcon{long-arrow-alt-right} V}:
        \item \textbf{II \faIcon{long-arrow-alt-right} I} and \textbf{II \faIcon{long-arrow-alt-right} VII}:
    \end{itemize}
\end{tcolorbox}

We have now, one by one, gone through possible extensions of what we considered a minimal graph for the six \glspl{lnl} I, II, III, IV, V and VII. For each possible additional arc we have assessed this new connection's performance, both visually and quantitatively using mainly the \acrlong{bic}. In this last subsection, we will investigate what effect adding two spread parameters at the same time to the base graph has on the model's performance. It will be interesting to see if the impacts of different new arcs on the metrics are independent of each other and hence cumulative, or if we might see synergies between two added spread parameters.
