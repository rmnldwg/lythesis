...
\begin{minipage}{1.0\textwidth}
    ...
    The complete model now includes the \glspl{lnl} I, II, III, IV, V and VII. They are connected as shown in \cref{fig:complete:graph}, based on the results in \cref{chap:graph}. For the treatment of the contralateral side, we employed the model $\mathcal{M}_\alpha$ that was introduced in \cref{chap:bilateral}. We also called this the ``mixing model'', because it uses the mixing parameter $\alpha$ that sets the probability rates for spread from the tumor to the contralateral \glspl{lnl} in case of an extension over the mid-sagittal plane to be a linear combination between the ipsilateral base probability rates and the contralateral ones for a lateralized tumor:
\end{minipage}
