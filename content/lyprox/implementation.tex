...
We used the popular free and open source \gls{css} framework \href{https://bulma.io}{\faIcon{external-link-alt}~Bulma}. Very little \gls{css} knowledge is required to employ it effectively, but it is still heavily customizable and extensible making it a safe choice for further development and potential design changes in the future.

Because we find the appearance of all default Bulma elements and components very appealing, we did not customize more than the default colors. For those, we chose the corporate design colors of the \gls{usz}. Initially, we decided to use these colors because we expected the website to be hosted under an official \gls{usz} domain and on dedicated hardware from our institution. However, the slow and bureaucratic process of our IT department forced us to host LyProX externally and under our own domain. Nonetheless, we find the \gls{usz} color scheme pleasing and modern-looking, so we did not change it.
