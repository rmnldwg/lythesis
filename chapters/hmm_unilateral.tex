...
\end{equation}
%
and then multiplies it with the respective observation probability vector, which is a column of the $\mathbf{B}$ matrix, to finally marginalize over all possible true hidden states at the time $t$ of diagnosis.
...
A diagnosis is often not complete, meaning that not all \glspl{lnl} might have been observed with all available diagnostic modalities. E.g., while a patient may have undergone a \gls{pet} scan to identify suspicious lymph nodes in the whole neck, \gls{fna} is only performed in a subset of \glspl{lnl}. Hence, we must be able to deal with “incomplete” observations for some \glspl{lnl}. To do so, we first introduce a new observation variable
%
\begin{equation}
    d_v^{\mathcal{O}} \in \{ 0, 1, \emptyset \}
\end{equation}
%
where $\emptyset$ indicates \emph{unobserved}. Furthermore, we define a \emph{match function}
%
\begin{equation}
    \operatorname{match}(\mathbf{d}, \mathbf{z}) := 
    \begin{cases}
        \text{true} & \text{if} \,\, d_v^{\mathcal{O}} = z_v^{\mathcal{O}} \vee d_v^{\mathcal{O}} = \emptyset ; \forall v,\mathcal{O} \\
        \text{false} & \text{else}
    \end{cases}
\end{equation}
%
