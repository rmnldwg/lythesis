...
\begin{equation}
    \begin{aligned}
        \Tilde{b}^\text{c}_v &\neq \Tilde{b}^\text{i}_v \\
        \Tilde{t}^\text{\,c}_e &= \Tilde{t}^\text{\,i}_e
    \end{aligned}
\end{equation}
%
...
\begin{enumerate}
    \item $\centernot{\text{s}} , \centernot{\epsilon}$: The tumor does not cross or touch the mid-sagittal plane and is thus clearly lateralized. The base spread probabilities are $\big\{ \Tilde{b}_v^\text{i} \big\}$ and $\big\{ \Tilde{b}_v^{\text{c},\centernot{\epsilon}} \big\}$.
    \item $\centernot{\text{s}} , \epsilon$: The tumor is lateralized, but crosses or touches the mid-sagittal plane. We will discuss how to define the spread probabilities to the contralateral side below.
    \item $\text{s} , \epsilon$: The tumor is symmetric w.r.t. to the sagittal plane, thus $\Tilde{b}_v^{\text{c},\text{s}} = \Tilde{b}_v^\text{i}$
\end{enumerate}

Note that $\text{s} \, (\centernot{\text{s}})$ and $\epsilon \, (\centernot{\epsilon})$ denote the two binary variables \emph{symmetric} (or \emph{not symmetric}) and \emph{extending} (or \emph{not extending}) over the mid-sagittal plane. \\
...
%
\begin{equation}
    \Tilde{b}_v^{\text{c} , \epsilon} = \alpha \cdot \Tilde{b}_v^\text{i} + (1 - \alpha) \cdot \Tilde{b}_v^{\text{c} , \centernot{\epsilon}}
\end{equation}
%
...
When using the learned parameters to predict the risk of a new patient, the set of parameters for the risk computation $\boldsymbol{\hat{\theta}}_R$ is compiled from the total set of inferred parameters $\boldsymbol{\hat{\theta}} = \big\{ \Tilde{b}_v^\text{i} , \Tilde{b}_v^{\text{c} , \centernot{\epsilon}} , \alpha, \Tilde{t}_e, p_T \big\}$, with \gls{lnl} $v \leq V$, $e \leq E$ and T-stage $T \in \{1,2,3,4\}$, depending on the risk factors the patient presents with at the time of diagnosis. We can look at a concrete example:
\end{document}