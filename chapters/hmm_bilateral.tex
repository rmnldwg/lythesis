...
\begin{equation}
    \begin{aligned}
        \Tilde{b}^\text{c}_v &\neq \Tilde{b}^\text{i}_v \\
        \Tilde{t}^\text{\,c}_v &= \Tilde{t}^\text{\,i}_v
    \end{aligned}
\end{equation}
%
...
We can infer that in case 2 the spread probabilities to the contralateral \glspl{lnl} must be between the ones for the clearly lateralized (1) and the symmetric (3) case. Hence, we could introduce a new "mixing" parameter $\alpha$ that defines the contralateral spread from tumor to \glspl{lnl} as a linear superposition between the two extremes:
%
\begin{equation}
...
When using the learned parameters to predict the risk of a new patient, the set of parameters for the computation is compiled from the total set of inferred parameters $\boldsymbol{\hat{\theta}} = \big\{ \Tilde{b}_v^\text{i} , \Tilde{b}_v^{\text{c} , \centernot{\text{e}}} , \alpha, \Tilde{t}_v, p_T \big\}$ depending on the risk factors - i.e. the lateralization of the tumor, T-stage, etc - the patient presents with at the time of diagnosis.
\end{document}