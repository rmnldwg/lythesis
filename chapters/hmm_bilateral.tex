...
When using the learned parameters to predict the risk of a new patient, the set of parameters for the risk computation $\boldsymbol{\hat{\theta}}_R$ is compiled from the total set of inferred parameters $\boldsymbol{\hat{\theta}} = \big\{ \Tilde{b}_v^\text{i} , \Tilde{b}_v^{\text{c} , \centernot{\text{e}}} , \alpha, \Tilde{t}_v, p_T \big\}$, with \gls{lnl} $v \in V$ and T-stage $T \in \{1,2,3,4\}$, depending on the risk factors the patient presents with at the time of diagnosis. We can look at a concrete example:
\end{document}